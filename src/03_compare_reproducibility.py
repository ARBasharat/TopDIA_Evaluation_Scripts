...
def remove_oxidated_proteoforms(proteoforms_original, ppm_tole=5E-6):
    proteoforms_bk = deepcopy(proteoforms_original)
    for p in proteoforms_bk:
        p.status = False
    oxidation_mass = 15.994915
    proton = 1.00727
    extend_oxidation_mass = [i * oxidation_mass for i in range(0, 101)]
    # cluster proteoforms by accession first
    ...
    #############################
    # cluster proteoforms by start and end idx of protein sequence
    new_proteoforms = []
    for key, prots in grouped_proteoforms.items():
        if len(prots) > 1:
            grouped_objects = {}
            for obj in prots:
                key = (obj.first_residue, obj.last_residue)
                if key not in grouped_objects:
                    grouped_objects[key] = []
                grouped_objects[key].append(obj)
            for key, objects in grouped_objects.items():
                if len(objects) > 1:
                    ...
                    for i in range(len(objects)):
                        ...
                        for j in range(i + 1, len(objects)):
                            obj2 = objects[j]
                            mass_diff = abs(obj1.mass - obj2.mass)
                            adjusted_mass_diff = abs(obj1.adjusted_mass - obj2.adjusted_mass)
                            # mass_diff_arr.append((i, j, mass_diff))
                            mass_tole = ppm_tole * max(obj1.mass, obj2.mass)
                            for m in extend_oxidation_mass:
                                if (abs(mass_diff - m) <= mass_tole or abs(adjusted_mass_diff - m) <= mass_tole or
                                    abs(adjusted_mass_diff - m + proton) <= mass_tole or abs(adjusted_mass_diff - m - proton) <= mass_tole or
                                        abs(mass_diff - m + proton) <= mass_tole or abs(mass_diff - m - proton) <= mass_tole):
                                    if obj1.mass <= obj2.mass:
                                        ...
                                        obj2.status = True
                                        break
                    # mass_differences[key] = mass_diff_arr
                    for i in range(len(objects)):
                        obj = objects[i]
                        if obj.status == False:
            ...
            new_proteoforms.append(prots[0])
    return new_proteoforms


def remove_oxidated_proteoforms_noSeqLength(proteoforms_original, ppm_tole=10E-6):
    proteoforms_bk = deepcopy(proteoforms_original)
    for p in proteoforms_bk:
        p.status = False
    oxidation_mass = 15.994915
...
def remove_duplicate_proteoforms_oxidations(proteoforms_original, ppm_tole=10E-6):
    proteoforms_bk = deepcopy(proteoforms_original)
    for p in proteoforms_bk:
        p.status = False
    IM = 1.00235
        ...
        if len(objects) > 1:
            ...
            for i in range(len(objects)):
                ...
                for j in range(i + 1, len(objects)):
                    ...
                    mass_tole = ppm_tole * max(obj1.mass, obj2.mass)
                    for m in extend_oxidation_mass:
                        if (abs(obj1.mass - obj2.mass - m) <= mass_tole or abs(obj1.mass - obj2.mass - m - IM) <= mass_tole or abs(obj1.mass - obj2.mass - m + IM) <= mass_tole or
                                abs(obj1.mass - obj2.mass + m) <= mass_tole or abs(obj1.mass - obj2.mass + m - IM) <= mass_tole or abs(obj1.mass - obj2.mass + m + IM) <= mass_tole or
                                abs(obj1.adjusted_mass - obj2.adjusted_mass - m) <= mass_tole or abs(obj1.adjusted_mass - obj2.adjusted_mass - m - IM) <= mass_tole or abs(obj1.adjusted_mass - obj2.adjusted_mass - m + IM) <= mass_tole or
                                abs(obj1.adjusted_mass - obj2.adjusted_mass + m) <= mass_tole or abs(obj1.adjusted_mass - obj2.adjusted_mass + m - IM) <= mass_tole or abs(obj1.adjusted_mass - obj2.adjusted_mass + m + IM) <= mass_tole):
