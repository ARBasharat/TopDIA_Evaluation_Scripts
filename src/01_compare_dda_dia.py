...
def get_proteoform(proteoform_file, prsm_dir, inte_dict=None):
...
def _getOverlappingProteoforms(temp_proteoforms, proteoform, time_tol):
    ...
    for p_idx in range(0, len(temp_proteoforms)):
        ...
        if abs(p.rt - proteoform.rt) < time_tol:
    ...
    if ms1_features is None:
        ...
        for p_idx in range(len(proteoforms)):
            ...
            for mf in ms1_features:
                if abs(round(mf.mass, 3) - round(p.mass, 3)) == 0 and mf.charge == p.charge:
                    t.append(p_idx)
                    p.rt_low = mf.rt_low
...
if __name__ == '__main__':
    ...
    for replicate in replicates:
        print("Processing Test Replicate:", replicate - 1)
        ...
        for mass_range_idx in range(0, len(mass_ranges)):
            ...
            print("DIA - DDA: common proteoforms", len(common_proteoforms), "- common proteins", common, "\n")
        ...
        print("DIA Proteoforms:", len(DIA_proteoforms_data), "- Proteins:", len(DIA_proteins))
        print("DDA Proteoforms:", len(DDA_proteoforms_data), "- Proteins:", len(DDA_proteins))
        ...
        print("DDA - DIA: common proteoforms", len(common_proteoforms), round((len(common_proteoforms)/len(DDA_proteoforms_data)) * 100, 3))
        print("DDA - DIA: common proteins", common, round((common/len(DDA_proteins)) * 100, 3))
        ...
        print("Matched Peaks per common Scan (DDA vs DIA):", dda_matched_peaks_per_scan, dia_matched_peaks_per_scan)
        print("Num Peaks per common Scan (DDA vs DIA):", dda_peaks_per_scan, dia_peaks_per_scan)
        print("Ratio Peaks per common Scan (DDA vs DIA):", round(dda_matched_peaks_per_scan /
        ...
        plt.Figure()
        sns.regplot(x='Number of masses', y='Number of matched masses', data=pd.DataFrame(
            {'Number of masses': DIA_num_peaks_temp, 'Number of matched masses': DIA_matched_peaks_shared_num}), marker='.', label="DIA")
        sns.regplot(x='Number of masses', y='Number of matched masses', data=pd.DataFrame(
            {'Number of masses': DDA_num_peaks_temp, 'Number of matched masses': DDA_matched_peaks_shared_num}), marker='.', label="DDA")
        plt.legend()
        # plt.savefig("01_matched_peaks_2d_" + str(replicate) + "_noOxidation.jpg", dpi=2000)
        plt.show()
        plt.close()

        # used for aggregate plot
    ...
    axes[0].bar(r, mean_dia_proteoforms_data, yerr=stddev_dia_proteoforms_data, capsize=3, alpha=0.5,
                color='r', width=width, edgecolor='black', label='DIA')
    axes[0].bar(r + width, mean_dda_proteoforms_data, yerr=stddev_dda_proteoforms_data, capsize=3, alpha=0.5,
                color='g', width=width, edgecolor='black', label='DDA')
    axes[0].bar(r + width*2, mean_common_proteoforms_data, yerr=stddev_common_proteoforms_data, capsize=3, alpha=0.6,
                color='orange', width=width, edgecolor='black', label='Shared')
    ...
    # Plot for Proteins
    axes[1].bar(r, mean_dia_proteins_data, yerr=stddev_dia_proteins_data, capsize=3, alpha=0.5,
                color='r', width=width, edgecolor='black', label='DIA')
    axes[1].bar(r + width, mean_dda_proteins_data, yerr=stddev_dda_proteins_data, capsize=3, alpha=0.5,
                color='g', width=width, edgecolor='black', label='DDA')
    axes[1].bar(r + width*2, mean_common_proteins_data, yerr=stddev_common_proteins_data, capsize=3, alpha=0.6,
                color='orange', width=width, edgecolor='black', label='Shared')
    ...
    combined_legend_labels = ['DIA', 'DDA', 'Shared']
    fig.legend(combined_legend_labels, loc='upper center', bbox_to_anchor=(0.5, 1.0), ncol=len(combined_legend_labels))
    plt.subplots_adjust(bottom=0.175)
    # plt.savefig("000_combined_frequency_" + str(replicate) + "_noOxidation.jpg", dpi=2000, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    plt.Figure()
    out = venn2(subsets=(dia_unique_proteoform_data[0], dda_unique_proteoform_data[0], common_proteoform_data[0]), set_labels=('DIA', 'DDA'))
    for text in out.set_labels:
        text.set_fontsize(14)
    for text in out.subset_labels:
        text.set_fontsize(16)
    plt.title('Proteoforms', fontsize=16)
    # plt.savefig("01_venn_Proteoforms_" + str(replicates[0]) + "_noOxidation.jpg", dpi=2000, bbox_inches='tight')
    plt.show()
    plt.close()

    plt.figure()
    out = venn2(subsets=(dia_unique_proteoform_data[1], dda_unique_proteoform_data[1], common_proteoform_data[1]), set_labels=('DIA', 'DDA'))
    for text in out.set_labels:
        text.set_fontsize(14)
    for text in out.subset_labels:
        text.set_fontsize(16)
    plt.title('Proteoforms', fontsize=16)
    # plt.savefig("01_venn_Proteoforms_" + str(replicates[1]) + "_noOxidation.jpg", dpi=2000, bbox_inches='tight')
    plt.show()
    plt.close()

    plt.figure()
    out = venn2(subsets=(dia_unique_protein_data[0], dda_unique_protein_data[0], common_protein_data[0]), set_labels=('DIA', 'DDA'))
    for text in out.set_labels:
        text.set_fontsize(14)
    for text in out.subset_labels:
        text.set_fontsize(16)
    plt.title('Proteins', fontsize=16)
    # plt.savefig("01_venn_Proteins_" + str(replicates[0]) + "_noOxidation.jpg", dpi=2000, bbox_inches='tight')
    plt.show()
    plt.close()

    plt.figure()
    out = venn2(subsets=(dia_unique_protein_data[1], dda_unique_protein_data[1], common_protein_data[1]), set_labels=('DIA', 'DDA'))
    for text in out.set_labels:
        text.set_fontsize(14)
    for text in out.subset_labels:
        text.set_fontsize(16)
    plt.title('Proteins', fontsize=16)
    # plt.savefig("01_venn_Proteins_" + str(replicates[1]) + "_noOxidation.jpg", dpi=2000, bbox_inches='tight')
    plt.show()
    plt.close()
